...
JFIFHeader ParseJFIFSegment(unsigned char* file_content, unsigned int& index) {
  ...
  if (jfif_header.thumbnail_horizontal_pixel_count_ != 0 &&
    ...
    BOOST_LOG_TRIVIAL(info) << "JFIF thumbnail not parsed, TO DO." << std::endl;
  }
  return jfif_header;
}
...
/**
 * \fn ScanHeader ParseScanHeader(unsigned char* file_content, unsigned int
 * &index)
 *
...
ScanHeader ParseScanHeader(unsigned char* file_content, unsigned int& index) {
  ...
  // TODO Check on the length of the header.
 ...
 * \param[in, out] index The offset of the cursor in the file.
 *
 * \return A pair containing the key and the QuantizationTable with the
 * information extracted from the file_content pointer. The function moves the
 * cursor of the number of byte read.
 */
